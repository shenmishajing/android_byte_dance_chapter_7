...
public class IJKPlayerActivity extends Activity implements View.OnClickListener {
    ...
    private class VolumeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("android.media.VOLUME_CHANGED_ACTION")) {
                mVolumeCurrent = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
                volumeSeekBar.setProgress(mVolumeCurrent);
            }
        }
    }
    ...
    private void init() {
        ...
        rl_bottom = (LinearLayout) findViewById(R.id.include_play_bottom);
        ...
        volumeSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                if (fromUser) {
                    ...
                    mVolumeCurrent = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
                    seekBar.setProgress(mVolumeCurrent);
                }
            }
        ...
        this.registerReceiver(volumeReceiver, intentFilter);
        ...
        switch (view.getId()) {
            case R.id.ijkPlayer:
                if (menu_visible == false) {
                    ...
                    Animator animator = AnimatorInflater.loadAnimator(getApplicationContext(), R.animator.show_button);
                    animator.setTarget(rl_bottom);
                    animator.start();
                    menu_visible = true;
                } else {
                    Animator animator = AnimatorInflater.loadAnimator(getApplicationContext(), R.animator.move_button);
                    animator.setTarget(rl_bottom);
                    animator.start();
